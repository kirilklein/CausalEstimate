...
def compute_matching_ate(Y: pd.Series, matching_df: pd.DataFrame,
                         treated_col: str='treated_pid', 
                         control_col: str='control_pid') -> float:
    """
    ...
    merged_df = matching_df.merge(Y.rename('treated_outcome'), left_on=treated_col, right_index=True)
    merged_df = merged_df.merge(Y.rename('control_outcome'), left_on=control_col, right_index=True)
    ...
    treated_outcomes = Y[avg_control_outcomes.index]  # Ensure we're aligning with grouped treated pids
