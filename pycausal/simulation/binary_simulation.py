...
def compute_ATE_theoretical_from_data(data: pd.DataFrame, beta: list):
    ...
    E_Y1 = logistic(beta[0] + beta[1] * 1 + beta[2] * data.X1 + beta[3] * data.X2 + beta[4] * data.X1 * data.X2).mean()
    E_Y0 = logistic(beta[0] + beta[1] * 0 + beta[2] * data.X1 + beta[3] * data.X2 + beta[4] * data.X1 * data.X2).mean()
    return E_Y1 - E_Y0   
