...
class TestMatching(unittest.TestCase):
    ...
    def test_match_optimal_basic(self):
        ...
        self.assertListEqual(list(result.columns), ['treated_pid', 'control_pid', 'distance'])
    ...
    def test_match_optimal_custom_columns(self):
        df = self.df.rename(columns={'treatment': 'treat', 'ps': 'propensity', 'PID': 'ID'})
        result = match_optimal(df, treatment_col='treat', ps_col='propensity', pid_col='ID')
        self.assertIsInstance(result, pd.DataFrame)
        self.assertListEqual(list(result.columns), ['treated_pid', 'control_pid', 'distance'])
