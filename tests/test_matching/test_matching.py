...
class TestMatchingEstimator(unittest.TestCase):
    ...
    def test_compute_matching_ate_basic(self):
        ...
        self.assertTrue(-20 < ate < 20)  # Assuming the effect is within a reasonable range

    def test_compute_matching_ate_custom_columns(self):
        Y = pd.Series(self.df['outcome'].values, index=self.df['PID'])
        matching_df = self.matching_result.rename(columns={'treated_pid': 'treated', 'control_pid': 'control'})
        ate = compute_matching_ate(Y, matching_df, treated_col='treated', control_col='control')
        self.assertIsInstance(ate, float)
